...
function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      text: "Hi! I'm the BMSCE Assistant. Ask me anything about admissions, departments, or campus life!",
      sender: "bot",
    },
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (!apiKey) {
        setMessages((prev) => [
          ...prev,
          { text: "System: API Key missing in .env", sender: "bot" },
        ]);
        setIsTyping(false);
        return;
      }
      ...
      const systemContext =
        "You are the BMSCE Assistant. Be concise. Founding: 1946. Location: Basavanagudi. Rank: NAAC A++.";
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] font-sans">
      ...
      {isOpen && (
        <div
          className="bg-white w-80 sm:w-96 rounded-2xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 duration-300 mb-4"
          style={{ height: "500px" }}
        >
          {/* Header */}
          ...
          >
            <input
              ...
              className="flex-1 bg-white/90 backdrop-blur-md text-gray-900 placeholder-gray-400 text-sm rounded-xl px-4 py-2.5 border border-gray-200 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/30 transition-all"
            />
            <button
              type="submit"
